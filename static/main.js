...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aJ, request.at, true);
		} catch (e) {
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{az: false, aB: r.aB, V: r.V, X: r.X, aJ: r.aJ, aT: r.aT, as: r.as, at: r.at}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aB: $elm$http$Http$emptyBody, V: r.V, X: _List_Nil, aJ: 'GET', aT: $elm$core$Maybe$Nothing, as: $elm$core$Maybe$Nothing, at: r.at});
};
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$Main$User = F3(
	function (id, name, avatar) {
		return {I: avatar, H: id, y: name};
	});
...
var $author$project$Main$getUsers = $elm$http$Http$get(
	{
		V: A2(
			$elm$http$Http$expectJson,
			$author$project$Main$GotUsers,
			$elm$json$Json$Decode$list($author$project$Main$userDecoder)),
		at: '/static/users.json'
	});
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2($author$project$Main$Loading, $author$project$Main$getUsers);
};
...
var $author$project$Main$LoadedUsers = function (a) {
	return {$: 2, a: a};
};
...
var $author$project$Main$areOrdersEmpty = function (orders) {
...
var $author$project$Main$Product = F4(
	function (id, name, description, image) {
		return {S: description, H: id, _: image, y: name};
	});
...
var $author$project$Main$getProducts = $elm$http$Http$get(
	{
		V: A2(
			$elm$http$Http$expectJson,
			$author$project$Main$GotProducts,
			$elm$json$Json$Decode$list($author$project$Main$productDecoder)),
		at: '/static/products.json'
	});
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$Order = F2(
	function (product, amount) {
		return {s: amount, q: product};
	});
var $author$project$Main$product2order = function (product) {
	return A2($author$project$Main$Order, product, 0);
};
var $author$project$Main$resetAmount = function (order) {
		...
		switch (msg.$) {
			case 0:
				...
				if (!result.$) {
					...
					switch (model.$) {
						case 1:
							return _Utils_Tuple2(
								$author$project$Main$LoadedUsers(users),
								$author$project$Main$getProducts);
						case 3:
							var products = model.b;
							var networkFailure = model.c;
							return _Utils_Tuple2(
								A3($author$project$Main$Loaded, users, products, false),
					...
					switch (model.$) {
						...
						case 3:
							...
							var products = model.b;
							return _Utils_Tuple2(
								A3($author$project$Main$Loaded, users, products, true),
								$elm$core$Platform$Cmd$none);
						default:
							return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
					}
			...
			case 1:
				...
				if (result.$ === 1) {
					...
					switch (model.$) {
						...
						case 4:
							var user = model.a;
							var oldOrders = model.b;
							var users = model.c;
							return $author$project$Main$areOrdersEmpty(oldOrders) ? _Utils_Tuple2(
								A3(
									$author$project$Main$ProductView,
									user,
									A2($elm$core$List$map, $author$project$Main$product2order, products),
									users),
			...
			case 3:
				...
				var newOrders = A2(
					$elm$core$List$map,
					function (o) {
						return _Utils_eq(o.q.H, order.q.H) ? _Utils_update(
							o,
							{s: o.s + 1}) : o;
					},
					orders);
				return _Utils_Tuple2(
					A3($author$project$Main$ProductView, user, newOrders, users),
					$elm$core$Platform$Cmd$none);
			case 5:
				var user = msg.a;
				var orders = msg.b;
				...
				return _Utils_Tuple2(
					A3(
						$author$project$Main$ProductView,
						user,
						A2($elm$core$List$map, $author$project$Main$resetAmount, orders),
						users),
					$elm$core$Platform$Cmd$none);
			case 6:
				var user = msg.a;
			...
			default:
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$batch(
						_List_fromArray(
							[$author$project$Main$getUsers, $author$project$Main$getProducts])));
		}
	});
var $author$project$Main$CommitOrder = F4(
	function (a, b, c, d) {
		return {$: 6, a: a, b: b, c: c, d: d};
	});
var $author$project$Main$GetUsers = {$: 4};
var $author$project$Main$ResetAmounts = F3(
	function (a, b, c) {
...
var $author$project$Design$grid = $elm$html$Html$div(
	_List_fromArray(
		[
...
var $author$project$Main$ClickedProduct = F4(
	function (a, b, c, d) {
...
var $author$project$Main$productView = F4(
	function (user, order, orders, users) {
		var productText = (!order.s) ? order.q.y : (order.q.y + (' x' + $elm$core$String$fromInt(order.s)));
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					$elm$html$Html$Events$onClick(
					A4($author$project$Main$ClickedProduct, user, order, orders, users)),
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$src(order.q._),
					...
					_List_fromArray(
						[
							$elm$html$Html$text(order.q.S)
						]))
				]));
	});
var $author$project$Main$ClickedUser = function (a) {
	return {$: 2, a: a};
};
var $elm$html$Html$br = _VirtualDom_node('br');
var $author$project$Main$userView = function (user) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Events$onClick(
				$author$project$Main$ClickedUser(user)),
				A2($elm$html$Html$Attributes$style, 'margin', '10px'),
				A2($elm$html$Html$Attributes$style, 'text-align', 'center')
			]),
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$src(user.I)
					]),
				_List_Nil),
				A2(
				$elm$html$Html$p,
...
var $author$project$Main$view = function (model) {
	switch (model.$) {
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'Strichliste 2.0' + (networkFailure ? ' *' : ''))
							])),
						$author$project$Design$grid(
						A2($elm$core$List$map, $author$project$Main$userView, users))
					]));
		case 0:
			return A2(
				$elm$html$Html$div,
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Events$onClick($author$project$Main$GetUsers)
							]),
						_List_fromArray(
							[
		...
		default:
			...
			var orders = model.b;
			var users = model.c;
			var resetVisible = $author$project$Main$areOrdersEmpty(orders) ? 'hidden' : 'visible';
			var products = A2(
				$elm$core$List$map,
				function (o) {
					return o.q;
				...
				orders);
			var confirmText = $author$project$Main$areOrdersEmpty(orders) ? 'Zurück' : 'Bestätigen';
			return A2(
				$elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						...
						_Utils_ap(
							A2(
								$elm$core$List$map,
								function (o) {
									return A4($author$project$Main$productView, user, o, orders, users);
								},
								orders),
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$Events$onClick(
											A4($author$project$Main$CommitOrder, user, orders, users, products))
										]),
									_List_fromArray(
										[
											$elm$html$Html$text(confirmText)
										])),
									A2(
									$elm$html$Html$button,
									_List_fromArray(
										[
											$elm$html$Html$Events$onClick(
											A3($author$project$Main$ResetAmounts, user, orders, users)),
											A2($elm$html$Html$Attributes$style, 'visibility', resetVisible)
										]),
									_List_fromArray(
										[
											$elm$html$Html$text('Zurücksetzen')
										]))
								])))
					]));
	}
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{aI: $author$project$Main$init, aS: $author$project$Main$subscriptions, aV: $author$project$Main$update, aW: $author$project$Main$view});
